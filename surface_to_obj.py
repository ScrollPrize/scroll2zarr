...
def simpleDecimateTest():
    ...
    testxyzs[:, 0, 2] = np.arange(0.0, testshape[0])
    testxyzs[:, 1, 2] = np.arange(0.0, testshape[0])
    testxyzs[1, 0, 0] = 2.0
    testxyzs[2, 0, 0] = 3.0
    testxyzs[4, 0, 0] = 5.0
    # testxyzs[1,0] = [0., 0., 2.]
    ...
    decimate(testxyzs, testflags, 0, 0.2, 0)
    print(testflags)
    decimate(testxyzs, testflags, 0, 0.2, 1)
    print(testflags)
    exit()
...
def gridDecimate(xyzuv, zstep, zsmoothing):
    ...
    if zsmoothing > 0:
        ...
        for i in range(3):
            ...
            xyzuv[:, :, i] = cv2.GaussianBlur(
                xyzuv[:, :, i], (0, 0), sigmas[0], sigmas[1], cv2.BORDER_ISOLATED
            )
...
def readVcps(vcps_path):
    ...
    expected = {"dim": "3", "ordered": "true", "type": "double", "version": "1"}
    ...
    if vcps_raw.shape[0] != vcps_width * vcps_height * 3:
        print(
            "Expected vcps file to contain",
            vcps_width * vcps_height * 3,
            "points, contains",
            vcps_raw.shape[0],
        )
        return None
...
class TrglList:
    ...
    @staticmethod
    def delaunayInternal(iuvpts, uvstep=0.0, nudge_uv=False):
        uvpts = iuvpts.copy()
        if nudge_uv:
            uvpts[:, 0] += 0.00001 * uvpts[:, 1]
            uvpts[:, 1] += 0.00001 * uvpts[:, 0]
        print("triangulating")
        if uvstep > 0.0:
    ...
    def fromDelaunay(uvpts):
        return TrglList.delaunayInternal(uvpts, 0.0, False)
    ...
    def connectedComponentLabels(self):
        ...
        # print("connections")
        # print(connections)
        nc, labels = scipy.sparse.csgraph.connected_components(
...
def parseSlices(istr, dim):
    ...
    if len(sstrs) != dim:
        print(
            "Could not parse range argument '%s'; expected %d comma-separated ranges"
            % (istr, dim)
        )
        return [None] * dim
    slices = []
...
def windowMinMax(istr, pts):
    owin = np.array([pts.min(axis=0) - 0.001, pts.max(axis=0) + 0.001])
    # print("owin", owin)
    wmin = owin[0, :]
    wmax = owin[1, :]
    if istr is None:
...
def main():
    ...
    parser.add_argument("input_surface_file", help="obj, ppm, or vcps file")
    parser.add_argument("output_obj_file", help="Name of output obj file")
    parser.add_argument(
        "--zstep",
        type=int,
        default=24,
